...
export default function AssemblyGame() {
  ...
  const numberOfGuessesLeft:number = languages.length - 1;

  const wrongGuessCount:number = guessedLetters.filter(
    (letter:string):boolean => !currentWord.toLowerCase().includes(letter)
  ).length;
  ...
  const isGameLost:boolean = wrongGuessCount >= numberOfGuessesLeft;
  const isGameOver:boolean = isGameWon || isGameLost;

  const lastGuessedLetter:string = guessedLetters[guessedLetters.length - 1];
  const isLastGuessIncorrect:boolean = !!(lastGuessedLetter && !currentWord.toLowerCase().includes(lastGuessedLetter));
